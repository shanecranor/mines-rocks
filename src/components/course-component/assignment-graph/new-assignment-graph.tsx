...
import { CourseSummaryData } from '../summary-data/new-summary-data';
// import { PaletteDemo } from '../../utils/colors';
export default function NewAssignmentGraph({
  courseData,
  // assignments,
  groupStats,
  totalWeight,
  ...
  const { isLoading, data: assignments } = useQuery({
    queryKey: [`courseAssignments${courseData.id}`],
    queryFn: async () => await getAssignmentsForCourse(String(courseData.id)),
    //cache for 10 days
    staleTime: 1000 * 60 * 60 * 24 * 10,
  });
  // get start and end dates
  if (isLoading) return <>loading assignments</>;
  if (!assignments) return <>no assignments found</>;
