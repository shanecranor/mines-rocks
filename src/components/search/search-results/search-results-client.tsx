...
type CourseAndComponent = {
  course: Course;
  bannerCourses: BannerCourse[];
  courseComponent: React.ReactNode;
};
...
function sortCourseList(courseList: any, searchOptions$: any) {
  const sortDirection = searchOptions$.sortOptions.isPrimarySortReversed.get()
    ? -1
    : 1;
  return courseList.sort((a: CourseAndComponent, b: CourseAndComponent) => {
    ...
    switch (sortValue) {
      ...
      case 'Course Number':
        const { deptCode: aDeptCode, courseNumber: aCourseNum } =
          splitCourseCode(getCourseAttributes(a.course).courseCode);
        const { deptCode: bDeptCode, courseNumber: bCourseNum } =
          splitCourseCode(getCourseAttributes(b.course).courseCode);
      ...
      case 'Course Name':
        ...
        return (
          b.course.name.localeCompare(a.course.name, undefined, {
            sensitivity: 'base',
          }) * sortDirection
        );
      ...
      case 'Total Enrollment':
        ...
        const { nonLabCourses: aNonLabCourses, labCourses: aLabCourses } =
          getLabAndNonLabCourses(a.bannerCourses);
        const { nonLabCourses: bNonLabCourses, labCourses: bLabCourses } =
          getLabAndNonLabCourses(b.bannerCourses);
...
function filterCourseList(courseList: any, searchOptions$: any) {
  ...
  return courseList.filter((courseAndComponent: CourseAndComponent) => {
    ...
    if (bannerCourses) {
      ...
      if (
        bannerCourseName &&
        bannerCourseName.toLowerCase().includes(searchText)
      ) {
        return true;
      }
    }
