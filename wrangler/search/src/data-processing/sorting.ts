...
export function sortCourseList(courseList: any, searchOptions$: any) {
	...
	return courseList.sort((a: ExtendedCourse, b: ExtendedCourse) => {
		...
		switch (sortValue) {
			...
			case 'Course Number':
				const { deptCode: aDeptCode, courseNumber: aCourseNum } = splitCourseCode(getCourseAttributes(a).courseCode);
				const { deptCode: bDeptCode, courseNumber: bCourseNum } = splitCourseCode(getCourseAttributes(b).courseCode);
