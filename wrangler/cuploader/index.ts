...
export default {
  async fetch(request: Request, { SUPABASE_URL, SUPABASE_SERVICE_ROLE }: Env) {
    ...
    const { responses, status } = await getResponses(routeInfoList, AUTH_TOKEN, urlParams);
    ...
    for (const routeInfo of routeInfoList) {
      ...
      if (typeof data === 'string') {
        return new Response(data, { status: StatusCodes.INTERNAL_SERVER_ERROR });
      };
      const { data: tableKeys } = await supabase.rpc(
        'list_columns', { table_id: supabaseTable }
      )
      ...
      responses[endpointName] = await upsertData(cleanedData, routeInfo.supabaseTable, supabase)
    }
