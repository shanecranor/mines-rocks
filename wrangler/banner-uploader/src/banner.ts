...
export class TermCodeError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "TermCodeError";
  }
}
...
export const getTermCode = async (termSeason: Season, termYear: string) => {
  ...
  const term = terms.find((term) =>
    term.description
      .toLowerCase()
      .includes(`${termSeason.toLowerCase()} ${termYear}`)
  );
  if (!term) {
    throw new TermCodeError(`Term ${termSeason} ${termYear} not found`);
  }
  return term.code;
};
...
export async function getCourseSearchResults(
  searchParams: { termCode: string; subject: string; courseNumber: string },
  cookies: string
) {
  ...
  if (!("data" in json)) {
    throw new Error(
      `Banner response does not contain data: ${JSON.stringify(json)}`
    );
  }
  return json.data;
}
...
export async function setTerm(
  termCode: string
): Promise<{ data: string; cookies: string }> {
